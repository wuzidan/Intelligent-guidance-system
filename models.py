...
class GKT(nn.Module):
    ...
    def _predict(self, h_next, qt):
        ...
        y = torch.clamp(y, min=-10, max=10)  # 防止 sigmoid 输入过大
    ...
    def forward(self, features, questions):
        ...
        for i in range(seq_len):
            ...
            if torch.isnan(xt).any() or torch.isinf(xt).any():
                print(f"Step {i} - xt: nan/inf, min={xt.min().item()}, max={xt.max().item()}")
            if torch.isnan(qt).any() or torch.isinf(qt).any():
                print(f"Step {i} - qt: nan/inf, min={qt.min().item()}, max={qt.max().item()}")
            ...
            if torch.isnan(yt).any() or torch.isinf(yt).any():
                print(f"Step {i} - yt: nan/inf, min={yt.min().item()}, max={yt.max().item()}")
