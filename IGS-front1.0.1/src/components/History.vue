...
const getRecordStats = (record) => {
    // 题目总数 = 记录中questions数组的长度
    const questionCount = record.questions.length;

    // 正确题目数量 = 筛选出correct为true的题目数量
    const correctCount = record.questions.filter(
        (question) => question.correct
    ).length;

    // 正确率 = (正确数量 / 总数量) * 100，保留1位小数（避免除0错误）
    const accuracy =
        questionCount > 0
            ? Math.round((correctCount / questionCount) * 1000) / 10 // 四舍五入保留1位小数
            : 0;

    return {
        questionCount, // 题目总数
        correctCount, // 正确题目数量
        accuracy, // 正确率（百分比，保留1位小数）
    };
};
